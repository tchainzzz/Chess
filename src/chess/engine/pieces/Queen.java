...
public class Queen extends Piece {
	...
	public Queen(final int piecePosition, final Side color) {
		super(PieceType.QUEEN, piecePosition, color, false);
	}

	public Queen(final int piecePosition, final Side color, final boolean hasMoved) {
		super(PieceType.QUEEN, piecePosition, color, hasMoved);
	}
	...
	@Override
	public Queen pieceFromMove(Move move) {
		int destination = BoardUtils.mailbox[move.getDestination()];
		int side = move.getMovedPiece().getSide().isWhite() ? 0 : 1;
		return (Queen) Piece.ALL_POSSIBLE_PIECES[destination][side][Piece.QUEEN_INDEX];
	}
}
